# PARTIAL MATCH _ SAME POINTS _ DYNAMIC ACTIVITY _ FINAL SCORE _ REVIEWER RECOMMENDATION
import sqlite3
import pandas as pd
...
def compute_absolute_similarity(new_pr_doc, reviewer_doc):
    ...
    using partial (substring) matching.
    
    For each token in the new PR document, if it is found as a substring in any token of the
    reviewer's document, the count for that token is added to the score.
    """
    # Tokenize both documents (lowercase and split on whitespace)
    ...
    # Sum frequencies for tokens in new_pr_doc that are found as substrings in any reviewer token
    abs_score = 0
    for new_token in new_tokens:
        for rtoken, count in freq_map.items():
            if new_token in rtoken:  # Partial match: new_token appears within rtoken
                abs_score += count
    return abs_score
