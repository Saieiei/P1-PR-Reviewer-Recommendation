...
def rank_reviewers(new_pr_doc, reviewers, vectorizer, tfidf_matrix,
                   pagerank_scores, activity_scores,
                   weight_sim_act=1.0,  # weight for sim*act
                   gamma=0.2           # weight for PageRank
                  ):
    """
    For the new PR document, compute cosine similarity to each reviewer's doc,
    combine with activity and pagerank scores using a multiplicative approach
    for similarity and activity, plus an additive term for PageRank:
        final_score = weight_sim_act * (sim_score * act_score) + gamma * pr_score
    ...
    for i, reviewer in enumerate(reviewers):
        sim_score = similarities[i]             # 0..1
        pr_score = pagerank_scores.get(reviewer, 0.0)  
        act_score = activity_scores.get(reviewer, 0.0) 
        # Multiplicative approach for sim & activity
        sim_act_product = sim_score * act_score
...
def main():
    ...
    ranked_reviewers = rank_reviewers(
        ...
        pagerank_scores, 
        activity_scores,
        weight_sim_act=1.0,  # or however you want
        gamma=0.2
    )
