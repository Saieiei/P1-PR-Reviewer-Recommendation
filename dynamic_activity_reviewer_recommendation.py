...
def compute_dynamic_activity_scores(reviews_df, tau=30):
    ...
    For each review:
      - Assign base points based on state (APPROVED=3, CHANGES_REQUESTED=2, COMMENTED=1, COMMIT=2).
      - Compute days_ago = (now - review_date) in days.
      - Decay factor = exp( -days_ago / tau ).
      - Partial score = base_points * decay factor.
    
    Sum these partial scores per reviewer and normalize the scores to [0,1].
    """
    # Convert review_date to datetime (tz-aware)
